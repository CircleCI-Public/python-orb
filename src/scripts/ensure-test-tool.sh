...
if [ -f ${REQUIREMENTS_PATH} ]; then
    ...
    echo "WARNING: No dependency file for ${PYTHON_ORB_PKG_MNGR} found. ${REQUIREMENTS_PATH} expected."
fi

# Automatically install test package. unittest is preinstalled and not required.
if [ "${PYTHON_ORB_TEST_TOOL}" != "unittest" ]; then
    DETECT_TEST_TOOL=$(eval "${PYTHON_ENV_TOOL:+$PYTHON_ENV_TOOL run} pip --disable-pip-version-check list" |
    awk 'NR > 2 { print $1 }' | grep "^${PYTHON_ORB_TEST_TOOL}$")
    
    # If the test package is not detected, install using PYTHON_INSTALL_TOOL
    if [ -z "$DETECT_TEST_TOOL" ]; then
        echo "INFO: Test package ${PYTHON_ORB_TEST_TOOL} was not found. Installing..."
        eval "${PYTHON_ENV_TOOL:-pip} install ${PYTHON_ORB_TEST_TOOL}"
    else
